...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10, zero_init_residual=False, groups=1,
        ...
        self.layer1 = self._make_layer(block, 16, layers[0]) if layers[0]!=-1 else nn.Sequential()
        self.layer2 = self._make_layer(block, 32, layers[1], stride=2) if layers[1]!=-1 else nn.Sequential()
        #self.layer3 = self._make_layer(block, 64, layers[2], stride=2) if layers[1]!=-1 else nn.Sequential()
...
def resnet12_56(c, pretrained=False, path=None, **kwargs):
    """
    ...
    model = ResNet(BasicBlock, [2, 3, 0], num_classes=c, **kwargs)
    if pretrained:
...
def resnet10_56(c, pretrained=False, path=None, **kwargs):
    """
    Constructs a ResNet-32 model.
    ...
    model = ResNet(BasicBlock, [2, 2, 0], num_classes=c, **kwargs)
    if pretrained:
        checkpoint = torch.load(path)
        state_dict = checkpoint['state_dict']
...
def resnet2_56(c, pretrained=False, path=None, **kwargs):
    """
    Constructs a ResNet-32 model.
    ...
    model = ResNet(BasicBlock, [-1, -1, -1], num_classes=c, **kwargs)
    if pretrained:
        checkpoint = torch.load(path)
        state_dict = checkpoint['state_dict']
...
def resnet4_56(c, pretrained=False, path=None, **kwargs):
    """
    Constructs a ResNet-32 model.
...
def resnet8_56(c, pretrained=False, path=None, **kwargs):
    ...
    model = ResNet(Bottleneck, [2, -1, -1], num_classes=c, **kwargs)
    if pretrained:
        checkpoint = torch.load(path)
        state_dict = checkpoint['state_dict']
